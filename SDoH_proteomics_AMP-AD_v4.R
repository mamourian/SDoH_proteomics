...
## ROSMAP clinical data w/ Subject ID - remove these IDs from AMP-AD dataset
clin_ROSMAP.orig <- read.csv(paste0(dir_ROSMAP, "proteomics/syn21261728/input/rosmap_50batch_specimen_metadata_for_batch_correction.csv"), stringsAsFactors = FALSE)
...
clin_id <- merge(clin.orig, id_map, by="individualID") %>%
  ...
  # dplyr::filter(!individualID %in% clin_ROSMAP.orig$IndividualID) %>% # would any ROSMAP subjects from Round 1 in AMP-AD dataset
...
################################################################################
### check subject/sample overlap with ROSMAP
clin_id_overlap <- merge(clin.orig, id_map, by="individualID") %>%
  dplyr::filter(specimenID %in% names(prot.orig)) %>%
  dplyr::filter(individualID %in% clin_ROSMAP.orig$IndividualID) %>% # selects ROSMAP subjects from Round 1 in AMP-AD dataset
  dplyr::filter(ADoutcome %in% c("AD", "Control"))
clin_id_sample <- clin_id_overlap %>%
  dplyr::select(individualID, specimenID) %>%
  dplyr::rename(IndividualID = individualID) %>%
  dplyr::rename(SampleID_AMPAD = specimenID)
clin_ROSMAP <- clin_ROSMAP.orig %>%
  dplyr::select(IndividualID, SampleID) %>%
  dplyr::rename(SampleID_ROSMAP= SampleID)

id_overlap <- merge(clin_id_sample, clin_ROSMAP)
write.csv(id_overlap, paste0(dir,"mamourie/ShenLab/SDoH/ROSMAP_ID_overlap.csv"), row.names = FALSE)
...
## Write output
write.csv(stats_df, paste0(dir,"mamourie/ShenLab/SDoH/SDoH_differentialAbundance_AMP-AD_v4.csv"), row.names = FALSE)
...
ggsave(
  file.path(paste0(dir,"mamourie/ShenLab/SDoH/SDoH_VolcanoPlot_AMP-AD_v4.png")),
